    ...
    def updateWindow(testenv):
        for actuatorName, actuatorVal in testenv.valpairs.items():
            if actuatorName in testenv.labelpairs.keys():
                ...
                label.config(text=f"{actuatorName}: {round(testenv.convertAngleToVal(actuatorName, actuatorVal), 1)} Steps, {round(actuatorVal, 1)} Degrees", font=("CourierPrime Regular", 18))
                label.pack()
        testenv.frame.pack()
